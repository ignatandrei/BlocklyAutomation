...
export class email {
  definitionBlocks(blocks: any, javaScript: any) {
    ...
    blocks['blockemail'] = {
      init: function () {
        this.appendValueInput('from').setCheck(null).appendField('From:');
        this.appendValueInput('to').setCheck(null).appendField('To:');
        this.appendValueInput('cc').setCheck(null).appendField('CC:');
        this.appendValueInput('bcc').setCheck(null).appendField('BCC:');
        this.appendValueInput('subject').setCheck(null).appendField('Subject:');
    ...
    blocks['sendinblue'] = {
      init: function () {
        this.appendDummyInput().appendField('SendInBlue');
        this.appendValueInput('APIKey').setCheck(null).appendField('ApiKey');
        ...
        this.setColour(230);
        this.setTooltip(
          'Please see https://account.sendinblue.com/advanced/api'
        );
    ...
    javaScript['sendinblue'] = function (block: any) {
      var value_apikey = javaScript.valueToCode(
        block,
        'APIKey',
        javaScript.ORDER_ATOMIC
      );
      var value_email = javaScript.valueToCode(
        block,
        'email',
        javaScript.ORDER_ATOMIC
      );
    ...
    javaScript['blockemail'] = function (block: any) {
      var value_from = javaScript.valueToCode(
        block,
        'from',
        javaScript.ORDER_ATOMIC
      );
      var value_to = javaScript.valueToCode(
        block,
        'to',
        javaScript.ORDER_ATOMIC
      );
      var value_cc = javaScript.valueToCode(
        block,
        'cc',
        javaScript.ORDER_ATOMIC
      );
      var value_bcc = javaScript.valueToCode(
        block,
        'bcc',
        javaScript.ORDER_ATOMIC
      );
      var value_subject = javaScript.valueToCode(
        block,
        'subject',
        javaScript.ORDER_ATOMIC
      );
      var dropdown_name = block.getFieldValue('NAME');
      var value_body = javaScript.valueToCode(
        block,
        'body',
        javaScript.ORDER_ATOMIC
      );
